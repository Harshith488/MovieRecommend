#installing all the packages and intiating the setup
...
#operating on the model considering the top 25 items of model
model_information = getModel(recommending_model)
class(model_information$sim)
str(model_information$sim)
image(model_information$sim[1:25, 1:25], main = 'Heatmap of first 25 rows and cols ')

# to get the sum of columns and rows
sum_rows <- rowSums(model_information$sim > 0)
table(sum_rows)
sum_rows

sum_cols <- colSums(model_information$sim > 0)

qplot(sum_cols, fill=I("steelblue"), col=I("red"))+ ggtitle("Distribution of the column count")

qplot(sum_rows, fill = I("green"), col = I("red")) + 
  ggtitle("Distribution of row count")

#now the actual prediction thing is going to happen
top_recommendations = 10 # set as a default thing
to_be_recommended = predict(object = recommending_model, 
                            newdata = testing_data, n = top_recommendations)
str(to_be_recommended)
to_be_recommended

#let us suppose for a user1 and user2

user1 <- to_be_recommended@items[[1]] # recommendation for the first user
user1
movies_user1 <- to_be_recommended@itemLabels[user1]
...
#the last part i.e matrix with recommendations for every user
recommendation_matrix <- sapply(to_be_recommended@items,
                                function(x){ as.integer(colnames(movie_ratings)[x]) }) # matrix with the recommendations for each user
dim(recommendation_matrix)
head(recommendation_matrix, 8)
